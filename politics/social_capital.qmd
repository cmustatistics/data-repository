...
date: July 24, 2025
description: County-level data combining social capital indices, income inequality measures, and economic indicators, inspired by Rodríguez-Pose, Lee, and Lipp's "Golfing with Trump" research on social capital, decline, and populism. Students must join this with the January 6th insurrectionists dataset to examine relationships between social cohesion, economic conditions, and political outcomes across US counties.
categories:
  - EDA
  - linear regression
  - data cleaning
...
You are expected to join this dataset with the [January 6th insurrectionists dataset](https://cmustatistics.github.io/data-repository/politics/jan-6-insurrectionists.html), which contains crucial outcome variables including Trump vote margins, white population change, employment rate change, and manufacturing job rate change. This combination allows for a comprehensive analysis of the relationships between social capital, economic conditions, and political outcomes at the county level.

## Data
Each row represents a US county. The dataset contains approximately 3,100+ counties across all 50 US states, though some counties may have missing values for certain variables due to data availability limitations.
...
| county_state_clean_gini | Full county and state name (e.g., "Jefferson County, Alabama") from the gini coefficient file |
...
| county_state_clean_social | Full county and state name (e.g., "Jefferson County, Alabama") from the social capital file|
...
Additional variables available after joining with January 6th dataset: Trump vote margins, white population change, manufacturing employment decline, and other demographic and economic indicators.
