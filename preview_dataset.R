...
#' @param dataset Either a dataframe or the name of a CSV file 
#'.  (in `data/` directory) to load and preview.
...
preview_datasets <- function(n = 20, datasets = rmarkdown::metadata$data$files) {
  ...
  if(!is.list(datasets)){  #case where only a vector of names is provided
    datasets<-sapply(datasets,function(d){preview_dataset(n,d)},simplify=FALSE)
  } 
