...
## Questions
1. Perform an EDA of the Fandango fan ratings, compare how much and how often the average differs from the presented aggregate score `Fandango_Stars`. Does the result following the rounding rules you would expect?

2. Visualize the difference between Fandango ratings (both `Fandango_Stars` and `Fandango_Ratingvalue`) and other major platforms. Showcase the systemic differences however you like (hint: you can also use the user vote counts). Can the differences between Fandango's and other platform's ratings be explained by the rounding rules? If not, what other factors might explain them?
