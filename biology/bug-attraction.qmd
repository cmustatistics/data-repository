...
Traps were set up at three different sites (one urban, two rural). Relevant
weather conditions were recorded. The experiment was repeated 32 times: 16 times
at the urban site and 8 times at each rural site. This allowed the researchers
to rotate which location (within an individual site) each trap was placed in,
and allowed them to collect data under varying weather and moon phases. (Since
the moon is bright in the night sky, the presence of a bright moon could
presumably affect the results.)
...
| Location 	| Location the light trap was placed. Site codes starting with `B` are urban (the UCLA Botanical Garden); site codes `L` and `S` were rural locations (a UCLA field station and a UCLA ranch). Each site has six separate traps, designated by a number. |
| Site 	| Site. `BG` = botanical garden, `LK` = La Kretz field station, `Stunt` = stunt ranch. |
...
: [bug-attraction.csv](../data/bug-attraction.csv)
